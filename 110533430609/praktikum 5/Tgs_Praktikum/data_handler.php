...
function data_handler($root){
	...
	if(mysql_num_rows($res)){
		...
		if(isset($_GET['act']) && $_GET['act'] != '' && cek_login()==true){
			...
			switch($_GET['act']) {
				...
				default:
					if(cek_login()==false){
						// Jika belum login, maka akan memanggil fungsi show_data dengan parameter $root
						show_data($root);
					}else{
						// Jika sudah login, maka akan memanggil fungsi show_admin_data dengan parameter $root
						show_admin_data($root);
					}
					break;
			}
...
function show_admin_data($root) { ?>
	...
	?>
		...
		<table width=700 cellpadding=4 cellspacing=0 class="tabel" style="text-align:left">
			...
			while($row = mysql_fetch_row($res)){
				...
				<tr class="<?php echo $bg;?>">
					...
					<td align="center">| <a href="<?php echo $root;?>&amp;act=edit&amp;id=<?php echo $id;?>">Edit</a> | <a href="<?php echo $root;?>&amp;act=del&amp;id=<?php echo $id;?>" onclick="return confirm('Hapus data dengan id <?php echo $id;?> (<?php echo $row[1];?>)?')">Hapus</a> |</td>
				</tr>
			<?php
...
/**
* Fungsi untuk menampilkan data pada client
* @param string root parameter menu
*/
...
function data_editor($root, $id = 0){
	...
	if (isset($_POST['nim']) && $_POST['nim'] && cek_login()==true) {
	...
	?>
		...
		<form action="" method="post">
			<table cellpadding=4 cellspacing=0 class="tabel" style="text-align:left">
				<tr>
					<td width=100>NIM*</td>
					<!-- Input box untuk nim -->
					<td> <input type="text" name="nim" size=10 value="<?php echo $nim;?>" class="text" /> </td>
				</tr>
				<tr>
					<td>Nama</td>
					<!-- Input box untuk nama -->
					<td> <input type="text" name="nama" size=40 value="<?php echo $nama;?>" class="text" /> </td>
				</tr>
				<tr>
					<td>Alamat</td>
					<!-- Input box untuk alamat -->
					<td> <input type="text" name="alamat" size=60 value="<?php echo $alamat;?>" class="text" /> </td>
				</tr>
				<tr>
					<td></td>
					<!-- Button submit untuk mengirim data -->
					<td><input type="submit" value="Submit" class="button" /><input type="button" value="Cancel" onclick="history.go(-1)" class="button" /></td>
				</tr>
			</table>
		</form>
		<br />
